 ...
 * Wrapper around an rclcpp::Node which ensures all instances share an exeuctor.
...
public:
  ...
  /**
   * \note Must NOT called until #InitROS has been called
   * \details Forwards arguments to the contructor for rclcpp::Node
...
Node::SharedPtr Node::Create(Args && ... args)
{
  ...
  // Add new node to the exeuctor so its callbacks are called
